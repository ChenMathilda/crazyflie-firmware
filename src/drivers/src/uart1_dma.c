...
static void USART_Config(uint32_t baudrate, uint8_t *dmaRxBuffer, uint32_t BUFFERSIZE);

void USART_DMA_Start(uint32_t baudrate, uint8_t *dmaRxBuffer, uint32_t BUFFERSIZE)
{
  // Setup Communication
  USART_Config(baudrate, dmaRxBuffer, BUFFERSIZE);
...
static void USART_Config(uint32_t baudrate, uint8_t *dmaRxBuffer, uint32_t BUFFERSIZE)
{
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr =(uint32_t)dmaRxBuffer ; 
